...
class Env(Communicator):
	...
	def reset(self, done, first):
		...
		for s in self.client_socks:
			msg = self.recv_msg(self.client_socks[s], 'MSG_TEST_NETWORK_SPEED')[0]
	...
	def _thread_network_testing(self, client_ip):
		msg = self.recv_msg(self.client_socks[client_ip], 'MSG_TEST_NETWORK_SPEED')[0]
	...
	def infer(self, thread_number, client_ips):
		...
		self.infer_state = {}
		for s in self.client_socks:
			msg = self.recv_msg(self.client_socks[s], 'MSG_INFER_SPEED')[0]
	...
	def _thread_infer_offloading(self, client_ip):
		for i in range(config.iteration[client_ip]):
			msg = self.recv_msg(self.client_socks[client_ip], 'MSG_LOCAL_ACTIVATIONS_CLIENT_TO_SERVER')[0]
...
class RL_Client(Communicator):
	...
	def initialize(self, split_layer):
		...
		msg = self.recv_msg(self.sock,'MSG_TEST_NETWORK_SPEED')[0][1]
