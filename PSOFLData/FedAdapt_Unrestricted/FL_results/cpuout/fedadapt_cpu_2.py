...
with open("fedadaptunresdata.txt", "r") as cpufile:
	...
	for x in filedata:
		if x[0:2] == "[[":
			...
			#print(x)
			counter += 1
	device_cpu_util = [[] for x in range(0, 10)]
	for y in range(1, 11):
		#device_cpu_util = [[] for x in range(1, 11)]
		for timestamps in timestamp_dev[y - 1]:
			...
			#for timestamps in timestamp_dev[y]:
				...
				print(origin_cpu_start, origin_cpu_end, y)
