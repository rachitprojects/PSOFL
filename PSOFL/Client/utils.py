...
#    min_time = list_of_time[min(range(len(list_of_time)), key = lambda i: abs(int(list_of_time[i][-2:])-int(time[-2:])))]
#    return min_time
...
def get_utilisation(cpu_start, cpu_end):
    with open("cpuout.txt", 'r') as cpudet:
        ...
        utils = max(cpu_utils) - min(cpu_utils)
...
#print(get_nearest("12:13:15", ["12:13:10", "12:13:16"]))
